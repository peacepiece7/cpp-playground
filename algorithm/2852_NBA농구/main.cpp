...
int timeToSec(string a) {
    int pos = a.find(":");
    int min = atoi(a.substr(0, pos).c_str());
    int sec = atoi(a.substr(pos+1, a.size()).c_str());
    return min*60+sec;
}

void printTimeBySec(int a) {
    string min = a/60 < 10 ? "0" + to_string(a/60): to_string(a/60); 
    string sec = a%60 < 10 ? "0" + to_string(a%60) : to_string(a%60);
    cout << min << ":" << sec << "\n";
}
...
int main() {
    ...
    while(t--) {
        cin >> scoreTeam >> cTime;
        int currentTime = timeToSec(cTime);

        if(teamA.score > teamB.score) {
            teamA.winTime += currentTime - prevTime;
        }else if (teamB.score > teamA.score) {
            teamB.winTime += currentTime - prevTime;
        }

        if(scoreTeam == 1) teamA.score++;
        else teamB.score++;
        prevTime = currentTime;
    }
    int currentTime = timeToSec("48:00");
    if(teamA.score > teamB.score) {
        teamA.winTime += currentTime - prevTime;
    }else if (teamB.score > teamA.score) {
        teamB.winTime += currentTime - prevTime;
    ...
    printTimeBySec(teamA.winTime);
    printTimeBySec(teamB.winTime);
}